...
function BlockPreview({ scene, ratio, ready, assetError, playing, onPlay, index }) {
  ...
  return <div className="block-preview-area">
    <div className={`block-stage-container ratio-${ratio.replace(':', '-')}`}><div className="block-stage" style={{ aspectRatio: ratio.replace(':', '/') }}><canvas ref={canvasRef} aria-label={`Voorbeeld blok ${index + 1}: één cijfer ${scene.digit}`} style={{ background: scene.bg }} />{!ready && <span className="loading-label">{assetError ? 'Cijfers niet geladen' : 'Cijfers laden…'}</span>}</div></div>
...
export function App() {
  ...
  const prepareAssets = useCallback(async () => {
    setReady(false); setAssetError('');
    try { await loadGlyphs(); setReady(true); }
    catch (error) { setAssetError(error.message || 'De cijfervormen konden niet worden geladen.'); }
  }, []);
  ...
  function undo() { if (!history.past.length) return; const next = history.past.at(-1); setHistory(h => ({ past: h.past.slice(0, -1), future: [project, ...h.future] })); projectRef.current = next; setProject(next); setPlayingId(null); }
  function redo() { if (!history.future.length) return; const next = history.future[0]; setHistory(h => ({ past: [...h.past, project], future: h.future.slice(1) })); projectRef.current = next; setProject(next); setPlayingId(null); }
  ...
  function addScene(afterIndex = project.scenes.length - 1, duplicate = false) {
    ...
    const next = duplicate ? { ...source, id: uid() } : { id: uid(), digit: String((Number(source.digit) + 1) % 10), effect: 'single', bg: PALETTES[(afterIndex + 1) % PALETTES.length].bg, fg: PALETTES[(afterIndex + 1) % PALETTES.length].fg, accent: '#009D8B', duration: 0.45, speed: 1, scale: 1 };
  ...
  function deleteScene(id) { if (project.scenes.length === 1) return; commit({ ...project, scenes: project.scenes.filter(s => s.id !== id) }); if (playingId === id) setPlayingId(null); }
  ...
  useEffect(() => {
    function keydown(e) { if (['INPUT', 'SELECT', 'TEXTAREA'].includes(e.target.tagName) || e.target.isContentEditable || exportOpen) return; if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'z') { e.preventDefault(); e.shiftKey ? redo() : undo(); } if (e.code === 'Space' && e.target.tagName !== 'BUTTON') { e.preventDefault(); const id = project.scenes.some(scene => scene.id === selectedId) ? selectedId : project.scenes[0].id; setPlayingId(current => current === id ? null : id); } }
    window.addEventListener('keydown', keydown); return () => window.removeEventListener('keydown', keydown);
  });
  function downloadProject() { const url = URL.createObjectURL(new Blob([JSON.stringify(project, null, 2)], { type: 'application/json' })); const a = document.createElement('a'); a.href = url; a.download = `${project.name || 'number-motion'}.json`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 5000); setToast('Project gedownload'); }
  ...
  return <div className="app-shell blocks-app">
    <header className="topbar"><a className="brand" href="#" onClick={e => e.preventDefault()} aria-label="Number Motion"><span className="brand-mark"><GridFourIcon size={22} weight="fill" /></span><span>number<span className="brand-light">motion</span><span className="brand-dot">.</span></span></a><span className="app-tag">Eén cijfer per blok. Jouw video.</span><div className="top-actions"><button className="primary-button" onClick={openExport} disabled={!ready}><DownloadSimpleIcon size={18} weight="bold" /> Exporteer montage</button></div></header>
    <main className="blocks-main"><div className="project-bar"><div className="project-heading"><div className="eyebrow">JOUW VIDEOBLOKKEN</div><input aria-label="Projectnaam" className="project-name" maxLength={60} value={project.name} onChange={e => commit({ ...project, name: e.target.value })} /></div><div className="project-actions"><span className="save-indicator"><span />{saveState}</span><div className="history-actions"><IconButton label="Ongedaan maken" disabled={!history.past.length} onClick={undo}><ArrowCounterClockwiseIcon size={18} /></IconButton><IconButton label="Opnieuw" disabled={!history.future.length} onClick={redo}><ArrowClockwiseIcon size={18} /></IconButton></div><IconButton label="Project openen" onClick={() => uploadRef.current.click()}><UploadSimpleIcon size={18} /></IconButton><IconButton label="Project opslaan als JSON" onClick={downloadProject}><DownloadSimpleIcon size={18} /></IconButton><input ref={uploadRef} type="file" hidden accept=".json,application/json" onChange={importProject} /></div></div>
      <div className="composition-note"><p>Elk blok animeert één cijfer. Bij export plakken we de blokken achter elkaar.</p><label className="ratio-picker"><select aria-label="Beeldverhouding" value={project.ratio} onChange={e => commit({ ...project, ratio: e.target.value })}><option value="16:9">16:9 · Liggend</option><option value="1:1">1:1 · Vierkant</option><option value="9:16">9:16 · Staand</option></select><CaretDownIcon size={13} /></label></div>
      {(recoveryRaw !== null || storageReadError !== null) && <div className="project-notice" role="alert"><p>{recoveryRaw !== null ? 'Je bewaarde project kon niet worden geopend. Het oorspronkelijke bestand blijft bewaard totdat je deze versie opslaat.' : 'Je browseropslag kon niet worden gelezen. Automatisch bewaren staat uit. Met “Deze versie bewaren” vervang je het opgeslagen project.'}</p><div><button type="button" className="text-button" onClick={recoveryRaw !== null ? downloadRecovery : downloadProject}>{recoveryRaw !== null ? 'Download bewaard bestand' : 'Download deze versie'}</button><button type="button" className="text-button" onClick={() => { setRecoveryRaw(null); setStorageReadError(null); }}>Deze versie bewaren</button></div></div>}
      {assetError && <div className="project-notice" role="alert"><p>{assetError}</p><button type="button" className="text-button" onClick={prepareAssets}>Cijfers opnieuw laden</button></div>}
      <div className="blocks-list">{project.scenes.map((scene, index) => { const start = blockStart; blockStart += scene.duration; const end = blockStart; return <section key={scene.id} className={`number-block ${selectedId === scene.id ? 'is-selected' : ''}`} aria-label={`Videoblok ${index + 1}`} onFocusCapture={() => setSelectedId(scene.id)}><header className="block-heading"><div className="block-title-group"><span className="block-label">{String(index + 1).padStart(2, '0')}</span><h2>Cijfer {scene.digit}</h2><span className="block-timecode">{start.toFixed(2)} – {end.toFixed(2)} s</span></div><div className="block-toolbar"><IconButton label={`Blok ${index + 1} omhoog`} disabled={index === 0} onClick={() => moveScene(index, -1)}><ArrowUpIcon size={16} /></IconButton><IconButton label={`Blok ${index + 1} omlaag`} disabled={index === project.scenes.length - 1} onClick={() => moveScene(index, 1)}><ArrowDownIcon size={16} /></IconButton><IconButton label={`Blok ${index + 1} dupliceren`} disabled={project.scenes.length >= 60} onClick={() => addScene(index, true)}><CopyIcon size={16} /></IconButton><IconButton label={`Blok ${index + 1} verwijderen`} disabled={project.scenes.length === 1} onClick={() => deleteScene(scene.id)}><TrashIcon size={16} /></IconButton></div></header>
        <div className="block-layout"><BlockPreview scene={scene} index={index} ratio={project.ratio} ready={ready} assetError={assetError} playing={playingId === scene.id} onPlay={() => { setSelectedId(scene.id); setPlayingId(id => id === scene.id ? null : scene.id); }} /><div className="block-settings">
          <div className="section-label"><h3>Cijfer</h3><span>Precies één in beeld</span></div><div className="digit-picker">{Array.from({ length: 10 }, (_, i) => <button key={i} aria-label={`Cijfer ${i}`} aria-pressed={String(scene.digit) === String(i)} className={String(scene.digit) === String(i) ? 'selected' : ''} onClick={() => patchScene(scene.id, { digit: String(i) })}>{i}</button>)}</div>
          <div className="block-motion-row"><label className="section-label" htmlFor={`effect-${scene.id}`}><h3>Animatie</h3></label><select id={`effect-${scene.id}`} className="effect-select" aria-label="Type animatie" value={scene.effect} onChange={e => patchScene(scene.id, { effect: e.target.value })}>{EFFECTS.map(effect => <option key={effect.id} value={effect.id}>{effect.label}</option>)}</select>
          ...
          <div className="block-colors"><ColorInput label="Achtergrond" value={scene.bg} onChange={bg => patchScene(scene.id, { bg })} /><ColorInput label="Cijferkleur" value={scene.fg} onChange={fg => patchScene(scene.id, { fg })} /><IconButton label="Kleuren wisselen" onClick={() => patchScene(scene.id, { bg: scene.fg, fg: scene.bg })}><ArrowsClockwiseIcon size={16} /></IconButton></div>
